...
float PIDController::update(float setpoint, float process_value) {
  ...
  ESP_LOGD("pid", "update: output=%f, wa=%f, samples=%d, inDeadband=%d, prop=%f, int=%f, der=%f, dt_=%f, kpm=%f, kim=%f, kdm=%f", 
    output, wa, samples, in_deadband(),
    proportional_term_, integral_term_, derivative_term_, dt_,
